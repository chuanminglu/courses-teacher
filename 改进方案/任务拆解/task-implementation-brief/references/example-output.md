> **本技能的形状基准，不是技术栈要求**：以下内容是 `task-implementation-brief` 的前身提示词一次真实展开的完整产出，用来锚定每次生成都必须达到的章节结构、字段颗粒度、证据分级用法（`[FACT]/[INFER]/[ASSUME]/[UNKNOWN]`）、"明确不要做的事"的详尽程度、以及《执行前必须确认清单》与"假设与待确认项"之间靠Q编号互相引用的呼应方式。文中出现的 Spring Boot / React / JdbcTemplate 等具体技术选型只是那次自查目标仓库的产物，生成新任务时必须按当次 `REPO_PATH` 自查结果重新判断技术栈与选型，不能因为示例是这个栈就照抄。
...
### 执行前必须确认清单
- [ ] **Q1**（默认生效-可反馈）数据库schema管理是否采用"新增`schema.sql`（`CREATE TABLE IF NOT EXISTS`）+ `spring.sql.init.mode: always`"，不引入Flyway等迁移框架？—— 对应「技术选型 #1」。若不反馈：按此方案执行。
- [ ] **Q2**（默认生效-可反馈）密码哈希是否使用`spring-security-crypto`的`BCryptPasswordEncoder`，不引入完整`spring-boot-starter-security`？—— 对应「技术选型 #2」。若不反馈：按此方案执行。
- [ ] **Q3**（默认生效-可反馈）会话凭证是否采用"自建`Session`表 + 服务端随机token（`Authorization: Bearer`头携带）"方案，而非JWT？—— 对应「技术选型 #3」。若不反馈：按此方案执行。
- [ ] **Q4**（默认生效-可反馈）是否在`AuthController`之外新增独立的`AuthService`承担业务逻辑分层？—— 对应「技术选型 #4」。若不反馈：按此方案执行。
- [ ] **Q5**（默认生效-可反馈）Feature Flag是否采用`@Value`直接注入`application.yml`属性，不新增`@ConfigurationProperties`封装类？—— 对应「技术选型 #5」。若不反馈：按此方案执行。
- [ ] **Q6**（默认生效-可反馈）Flag关闭时接口是否返回`404 Not Found`（视为路由不存在），而非`503`或自定义禁用提示？—— 对应「技术选型 #6」/DoD第3条。若不反馈：按此方案执行。
- [ ] **Q7**（默认生效-可反馈）时间可测试性是否通过注入`Clock`实现（不直接调用`LocalDateTime.now()`），为T00103预留mock能力？—— 对应「技术选型 #7」。若不反馈：按此方案执行。
- [ ] **Q8**（默认生效-可反馈）测试用户数据是否由集成测试内部调用`UserRepository`插入，不新增`data.sql`？—— 对应「技术选型 #8」。若不反馈：按此方案执行。
...
### 假设与待确认项
- **Q1～Q8** 密码哈希/会话凭证/Service分层/Flag实现方式/Flag关闭行为/时间可测试性/测试数据来源等技术选型，均基于代码库现状与业界惯例选定，材料未直接指定，如实现方计划采用其他方案（如换JWT、换完整Spring Security）需开工前反馈，避免T00102～T00106连锁返工；其中Q6（flag关闭返回404而非503/自定义禁用响应）如产品期望更明确的"功能未开放"提示，需调整
...
### 执行前必须确认清单
- [ ] **Q1**（阻塞）T00101是否已实际合并到主干？—— 对应「背景」/「明确不要做的事」。若不回答：无法开工，需先确认T00101已合并。
- [ ] **Q2**（阻塞）T00101实际交付的`AuthService`失败分支抛出方式，是否与本说明假设的"直接抛出笼统异常"一致？—— 对应「假设与待确认项」NEEDS CLARIFICATION项。若不回答：无法开工，需先核对T00101真实实现，若不一致需相应调整落点。
- [ ] **Q3**（默认生效-可反馈）失败传递机制是否采用"新增`AuthException`（携带`code`字段）+ `@ExceptionHandler`统一转换响应"？—— 对应「技术选型 #1」。若不反馈：按此方案执行。
- [ ] **Q4**（默认生效-可反馈）本任务是否暂不做时序攻击防护（两类失败响应耗时可能有差异）？—— 对应「假设与待确认项」第2条。若不反馈：按此方案（不处理）执行。
...
### 假设与待确认项
- **Q2** `NEEDS CLARIFICATION`：T00101实际交付后，`AuthService`失败分支的真实抛出方式需要核对，若T00101采用了不同于"直接return一个笼统异常"的实现（比如返回`Optional`/自定义Result类），本任务需要相应调整落点，而非硬套本说明的`AuthException`机制。
- **Q4** 未做时序攻击防护（两类失败响应耗时可能有差异，理论上可被用于判断账号是否存在），AC-2原文未提及安全合规要求，暂不处理，如有合规要求需另立任务。
...
### 执行前必须确认清单
- [ ] **Q1**（阻塞）T00101、T00102是否均已实际合并到主干？—— 对应「背景」/「明确不要做的事」。若不回答：无法开工，需先确认两者已合并。
- [ ] **Q2**（阻塞）T00101、T00102真实交付后，`AuthService`内部结构（尤其是异常携带状态码的方式）是否与本说明假设一致？—— 对应「假设与待确认项」NEEDS CLARIFICATION项。若不回答：无法开工，需先核对真实结构，若不同需相应调整落点（423状态码、字段命名、四条隐含规则本身不受影响）。
- [ ] **Q3**（默认生效-可反馈）锁定响应是否采用"`423 Locked` + `{ "error": "ACCOUNT_LOCKED", "lockedUntil": "<ISO8601>" }`"格式，返回绝对时间戳而非剩余秒数？—— 对应「技术选型 #2」。若不反馈：按此方案执行。
- [ ] **Q4**（默认生效-可反馈）锁定相关字段命名是否采用`failedLoginAttempts`/`lockedUntil`？—— 对应「假设与待确认项」第2条。若不反馈：按此命名执行。
...
### 假设与待确认项
- **Q2** `NEEDS CLARIFICATION`：T00101、T00102真实交付后，`AuthService`内部结构（尤其是异常携带状态码的方式）需要核对，若与本说明假设不同需调整落点，但423状态码、字段命名、四条隐含规则本身不受影响。
- **Q4** 锁定相关字段命名（`failedLoginAttempts`/`lockedUntil`）与响应格式为本说明选定，材料未规定具体命名规范。
...
### 执行前必须确认清单
- [ ] **Q1**（阻塞）T00101～T00103是否均已实际合并到主干？—— 对应「背景」/「明确不要做的事」。若不回答：无法开工，需先确认三者已合并。
- [ ] **Q2**（阻塞）`AuthService.login`方法签名与`SessionRepository.create`的真实参数是否与本说明假设一致？—— 对应「假设与待确认项」NEEDS CLARIFICATION项。若不回答：无法开工，需先核对T00101～T00103真实交付结果。
...
### 假设与待确认项
- **Q2** `NEEDS CLARIFICATION`：T00101～T00103真实交付后，`AuthService.login`方法签名与`SessionRepository.create`的真实参数需要核对。
...
### 执行前必须确认清单
- [ ] **Q1**（阻塞）T00101～T00104是否均已实际合并到主干？—— 对应「背景」/「明确不要做的事」。若不回答：无法开工，需先确认四者已合并。
- [ ] **Q2**（阻塞）`AuthService`四个分支（成功/账号不存在/密码错误/锁定）的真实代码位置是否与本说明假设一致？—— 对应「假设与待确认项」NEEDS CLARIFICATION项。若不回答：无法开工，需先核对T00101～T00104真实交付结果。
- [ ] **Q3**（默认生效-可反馈）`LoginEvent`是否只保证`eventType`/`username`/`createdAt`三个最基本字段，不等运营看板给出完整字段清单？—— 对应「技术选型 #2」。若不反馈：按此最小字段集执行，运营看板后续如需更多字段需另立任务补充。
...
### 假设与待确认项
- **Q3** 运营看板实际消费`LoginEvent`所需的完整字段清单，材料未说明，当前只保证`eventType`/`username`/`createdAt`三个最基本字段。
- **Q2** `NEEDS CLARIFICATION`：T00101～T00104真实交付后，`AuthService`四个分支的真实代码位置需要核对。
...
### 执行前必须确认清单
- [ ] **Q1**（阻塞）T00101～T00105是否均已实际合并到主干？—— 对应「背景」/「明确不要做的事」/「你要做什么」前提。若不回答：无法开工，本任务在T00101～T00105实际合并前不应被派发执行。
- [ ] **Q2**（阻塞）拦截器/校验组件的确切实现方式与文件位置，是否与本说明"预计涉及文件"假设的`AuthInterceptor`一致？—— 对应「假设与待确认项」NEEDS CLARIFICATION项。若不回答：无法开工，需以T00101～T00105实际交付代码库为准重新自查。
- [ ] **Q3**（默认生效-可反馈）T00101～T00105期间是否存在针对该flag专门写的单元测试（如"flag关闭时保持旧行为"用例）？—— 对应「预计涉及文件 · 可能改动」。若不反馈：按"若存在则随本任务一并删除/改写，若不存在无需额外动作"处理。
...
### 假设与待确认项
- **Q2** `NEEDS CLARIFICATION`：T00101～T00105实际交付后，拦截器/校验组件的确切实现方式与文件位置需要重新核实，本说明"预计涉及文件"部分列出的具体路径不能直接照搬，需以当时代码库为准重新自查。
- **Q3** 是否存在为该flag专门写过的针对性测试用例，材料未说明，需在真实交付后确认。
